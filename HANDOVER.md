...
4. 日次ギフト、Treasury、Palace Relief、ミッション、FORTUNE ECHOをそれぞれ一度実行し、Bot の取引ログと残高が一度だけ変化すること。
...
- Party Crown などの Lux 内の追加報酬を RIS 化する場合は、クライアント起点の報酬請求を追加せず、サーバー側で達成条件を検証する API を追加する。
