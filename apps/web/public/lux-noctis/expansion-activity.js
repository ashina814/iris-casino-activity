(function () {
  ...
  function patchArcana(game) {
    if (game.irisEconomyPatched) return; game.irisEconomyPatched=true;
    const glyph={moon:"M",star:"S",rose:"R",diamond:"D",crown:"C",wild:"W",sun:"O",eye:"E"}; const apply=(round)=>{const s=round.state;game.remoteRoundId=round.id;if(Array.isArray(s.cards)){game.cards=s.cards.map((symbol,id)=>({id,symbol:glyph[symbol]||symbol}));}else{game.cards=Array.from({length:16},(_,id)=>game.cards[id]||({id,symbol:"?"}));Object.entries(s.visibleCards||{}).forEach(([id,symbol])=>{const index=Number(id);if(Number.isInteger(index)&&index>=0&&index<game.cards.length)game.cards[index]={id:index,symbol:glyph[symbol]||symbol};});}game.open=s.open||[];game.matched=new Set(s.matched||[]);game.moves=s.moves||0; if(round.phase==="settled"){game.phase="result";game.status=round.payout?"ARCANA COMPLETE":"TIME OUT";recordRemote("arcana",round.id,game.bet,round.payout||0,round.payout&&s.moves<=10?"PERFECT ARCANA":"ARCANA MATCH",`${s.moves} MOVES`,round.payout&&s.moves<=10?[{event:"arcanaPerfect"}]:[]);}setWallet(round.wallet);};
