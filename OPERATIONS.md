...
- read-only確認: `npm run casino:status`、特定ユーザーの進行中roundは `npm run casino:active -- --user <DiscordID>` を使う。`reconciliationFailure: true` は手動再開せずincidentとして扱う。JSONを直接編集しない。
