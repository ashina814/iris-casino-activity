...
export function createApp(options: CreateAppOptions = {}) {
  ...
  app.post("/api/economy/artifacts/migrate", asyncRoute(async (req, res) => { const user = getSession(req).user; const body = z.object({ owned: z.array(z.string().max(32)).max(48) }).safeParse(req.body); if (!user) throw new AppError(401, "unauthorized", "Authentication is required."); if (!body.success) throw new AppError(400, "bad_request", "Artifact migration is invalid."); res.json({ ok: true, artifacts: await activityEconomy.migrateArtifacts(user, body.data.owned) }); }));
  ...
  app.post("/api/duel/claim", asyncRoute(async (req,res)=>{const user=getSession(req).user;if(!user)throw new AppError(401,"unauthorized","Authentication is required.");try{const matchId=String(req.body?.matchId||"");const claimed=duels.claim(user.id,matchId);const result=claimed.result==="win"||claimed.result==="tie"?claimed.result:"loss";const medals=result==="win"?5:result==="tie"?2:1;const reward=claimed.alreadyClaimed?{amount:0,wallet:null,currency:"Ris"}:await activityEconomy.claimDuel(user,matchId,claimed.amount);const season=claimed.alreadyClaimed?null:activityEconomy.awardDuelSeason(user,result);duels.markClaim(user.id,matchId);res.json({ok:true,alreadyClaimed:claimed.alreadyClaimed,result,reward:{coins:reward.amount,medals},wallet:reward.wallet,currency:reward.currency,season})}catch(error){throw new AppError(409,"casino_transaction_conflict",error instanceof Error?error.message:"Duel reward failed.")}}));
